# Virtual Intelligent Dev Team：研发 / 产品 / 技术治理闭环说明
...
> 一个面向研发 / 产品 / 技术治理复杂任务的、可验证、可回退、有边界的工作闭环 skill。
...
当用户要"先规划再开发"、大规模重写、迁移、架构改造时，先进入轻量 planning branch：

- 锁定范围、目标与约束
- 产出 analysis / plan / progress pack
- 建立可恢复的进度锚点
- 再回到正常执行路由

这让长周期改造有稳定的恢复点，而不是边做边猜。
...
当请求是小型功能、bugfix 或明确实现切片时：

- 用 quick slice brief 记录意图、非目标、验收标准
- 保留持久的项目上下文（project context）
- 产出交付状态与验证证据
- 不展开成完整产品 brief 或规划分支

这让小任务不至于过度仪式化，也不会缺少可追溯上下文。
...
对纯商业战略、定价、融资、泛行业咨询，这个 skill 不再假装自己是最佳主路由。边界不清时，应该先澄清，而不是强行接管。
