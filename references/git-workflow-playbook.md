...
- 通过条件：分支明确、无未解决冲突、无遗留暂存集；未暂存和未跟踪改动需要被明确记录。
- 失败处理：立即停止，先清理遗留暂存集或分支错误。
...
4. `G3 同步`
- 执行要点：先检查 upstream 关系是否清晰，再执行同步动作，优先小步解决冲突。
- 通过条件：本地分支与远端 ahead/behind 关系清晰、无未解决冲突，可安全执行同步。
- 失败处理：upstream 缺失或冲突无法判断时立即停止并请求人工决策。
- 校验命令：`python scripts/git_workflow_guardrail.py --repo . --stage G3 --pretty`

5. `G4 推送/PR`
- 执行要点：推送分支并按门禁发起评审流程。
- 通过条件：推送前已完成同步检查、`behind=0` 且 `ahead>0`，随后再执行推送或开 PR。
- 失败处理：权限不足、保护分支阻断或仍落后远端时停止并反馈阻塞点。
