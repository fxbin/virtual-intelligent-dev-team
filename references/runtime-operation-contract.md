...
- Before claiming `done`, `ready`, `fixed`, `committed`, `merged`, or handoff-ready, use `python scripts/verify_action.py --text "<user request>" --check completion-evidence --completion-evidence .skill-evidence/completion-evidence.json --pretty` to confirm structured completion evidence exists and supports the claim.
