...
- 发布后反馈闭环
- 显式自动运行与可恢复状态
- Worker / Verifier 对抗式质量闭环

## 运行流程图

```mermaid
flowchart TD
    A[复杂软件任务进入系统] --> B{是否显式输入 /auto}
    B -->|否| C[手动模式]
    B -->|是| D["/auto setup"]
    ...
    H --> I{是否大型改造 / 迁移 / 先规划}
    I -->|是| J["plan-first-build 规划闭环和 progress anchor"]
    J --> H
    I -->|否| K{是否窄实现或 bugfix}
    K -->|是| L[quick-slice-deliver 交付闭环]
...
3. 交付闭环
   - 小切片实现用 quick slice brief、持久项目上下文、交付状态、目标验证和自审，保证窄范围不漂移。
4. 迭代闭环
