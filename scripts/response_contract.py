...
def validate_beta_simulation_diff(payload: dict[str, object]) -> None:
    validate_payload_against_schema(
        payload,
...
def validate_beta_cohort_plan(payload: dict[str, object]) -> None:
    validate_payload_against_schema(
        payload,
        schema_path=BETA_COHORT_PLAN_SCHEMA_JSON_PATH,
        label="beta cohort plan",
    )
