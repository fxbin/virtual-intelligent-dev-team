#!/usr/bin/env python3
"""Initialize technical-governance workspace anchors for virtual-intelligent-dev-team."""
...
def init_technical_governance(root: Path, overwrite: bool = False) -> dict[str, object]:
    specs = [
        (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Initialize technical-governance workspace anchors.")
...
def main() -> None:
    args = parse_args()
    result = init_technical_governance(Path(args.root).resolve(), overwrite=args.overwrite)
    if args.pretty:
